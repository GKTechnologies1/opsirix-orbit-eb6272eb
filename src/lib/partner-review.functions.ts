...
export const reviewPartnerApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!canReview) return { success: false as const, error: "You do not have permission to review applications." };
    ...
    const { data: current, error: readError } = await context.supabase
      ...
      .maybeSingle();
    if (readError || !current) return { success: false as const, error: "Application not found." };

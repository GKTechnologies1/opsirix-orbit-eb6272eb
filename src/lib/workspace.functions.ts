...
export const getCompanyWorkspaces = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return { opx, organizations, members: (members.data ?? []).map((m) => ({ ...m, person: (memberPeople ?? []).find((p) => p.id === m.user_id) ?? null })), events: events.data ?? [], grants: (grants.data ?? []).map((grant) => ({ ...grant, person: (staffPeople ?? []).find((person) => person.id === grant.staff_user_id) ?? null })), userId: context.userId };
  });
